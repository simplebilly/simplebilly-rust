/*
 * SimpleBilly API
 *
 * Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.
 *
 * The version of the OpenAPI document: 0.1.0
 * Contact: support@simplebilly.com
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// InstituteType : Institutsart (KWG § 1).
/// Institutsart (KWG § 1).
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum InstituteType {
    #[serde(rename = "kein")]
    Kein,
    #[serde(rename = "kreditinstitut")]
    Kreditinstitut,
    #[serde(rename = "finanzdienstleistungsinstitut")]
    Finanzdienstleistungsinstitut,
    #[serde(rename = "finanzunternehmen")]
    Finanzunternehmen,
    #[serde(rename = "versicherung")]
    Versicherung,

}

impl std::fmt::Display for InstituteType {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Self::Kein => write!(f, "kein"),
            Self::Kreditinstitut => write!(f, "kreditinstitut"),
            Self::Finanzdienstleistungsinstitut => write!(f, "finanzdienstleistungsinstitut"),
            Self::Finanzunternehmen => write!(f, "finanzunternehmen"),
            Self::Versicherung => write!(f, "versicherung"),
        }
    }
}

impl Default for InstituteType {
    fn default() -> InstituteType {
        Self::Kein
    }
}

