/*
 * SimpleBilly API
 *
 * Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.
 *
 * The version of the OpenAPI document: 0.1.0
 * Contact: support@simplebilly.com
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct CouponValidation {
    #[serde(rename = "code")]
    pub code: String,
    #[serde(rename = "discount_type")]
    pub discount_type: String,
    #[serde(rename = "discount_value")]
    pub discount_value: String,
    #[serde(rename = "discounted_amount")]
    pub discounted_amount: String,
    #[serde(rename = "max_discount_amount", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub max_discount_amount: Option<Option<String>>,
    #[serde(rename = "reason", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub reason: Option<Option<String>>,
    #[serde(rename = "valid")]
    pub valid: bool,
}

impl CouponValidation {
    pub fn new(code: String, discount_type: String, discount_value: String, discounted_amount: String, valid: bool) -> CouponValidation {
        CouponValidation {
            code,
            discount_type,
            discount_value,
            discounted_amount,
            max_discount_amount: None,
            reason: None,
            valid,
        }
    }
}

