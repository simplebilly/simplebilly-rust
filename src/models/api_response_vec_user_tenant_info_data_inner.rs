/*
 * SimpleBilly API
 *
 * Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.
 *
 * The version of the OpenAPI document: 0.1.0
 * Contact: support@simplebilly.com
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct ApiResponseVecUserTenantInfoDataInner {
    #[serde(rename = "custom_domain", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub custom_domain: Option<Option<String>>,
    #[serde(rename = "role")]
    pub role: String,
    #[serde(rename = "subdomain", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub subdomain: Option<Option<String>>,
    #[serde(rename = "tenant_id")]
    pub tenant_id: uuid::Uuid,
    #[serde(rename = "tenant_name")]
    pub tenant_name: String,
}

impl ApiResponseVecUserTenantInfoDataInner {
    pub fn new(role: String, tenant_id: uuid::Uuid, tenant_name: String) -> ApiResponseVecUserTenantInfoDataInner {
        ApiResponseVecUserTenantInfoDataInner {
            custom_domain: None,
            role,
            subdomain: None,
            tenant_id,
            tenant_name,
        }
    }
}

