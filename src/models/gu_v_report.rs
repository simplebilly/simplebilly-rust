/*
 * SimpleBilly API
 *
 * Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.
 *
 * The version of the OpenAPI document: 0.1.0
 * Contact: support@simplebilly.com
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct GuVReport {
    #[serde(rename = "expenses")]
    pub expenses: Vec<models::GuVItem>,
    #[serde(rename = "generated_at")]
    pub generated_at: String,
    #[serde(rename = "net_income")]
    pub net_income: String,
    #[serde(rename = "period")]
    pub period: String,
    #[serde(rename = "revenue")]
    pub revenue: Vec<models::GuVItem>,
    #[serde(rename = "total_expenses")]
    pub total_expenses: String,
    #[serde(rename = "total_revenue")]
    pub total_revenue: String,
}

impl GuVReport {
    pub fn new(expenses: Vec<models::GuVItem>, generated_at: String, net_income: String, period: String, revenue: Vec<models::GuVItem>, total_expenses: String, total_revenue: String) -> GuVReport {
        GuVReport {
            expenses,
            generated_at,
            net_income,
            period,
            revenue,
            total_expenses,
            total_revenue,
        }
    }
}

