/*
 * SimpleBilly API
 *
 * Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.
 *
 * The version of the OpenAPI document: 0.1.0
 * Contact: support@simplebilly.com
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// DocumentType : Document kind of the invoice record.
/// Document kind of the invoice record.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum DocumentType {
    #[serde(rename = "invoice")]
    Invoice,
    #[serde(rename = "creditnote")]
    Creditnote,
    #[serde(rename = "advancepayment")]
    Advancepayment,
    #[serde(rename = "downpaymentinvoice")]
    Downpaymentinvoice,
    #[serde(rename = "deliverynote")]
    Deliverynote,
    #[serde(rename = "orderconfirmation")]
    Orderconfirmation,
    #[serde(rename = "quotation")]
    Quotation,
    #[serde(rename = "proformainvoice")]
    Proformainvoice,

}

impl std::fmt::Display for DocumentType {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Self::Invoice => write!(f, "invoice"),
            Self::Creditnote => write!(f, "creditnote"),
            Self::Advancepayment => write!(f, "advancepayment"),
            Self::Downpaymentinvoice => write!(f, "downpaymentinvoice"),
            Self::Deliverynote => write!(f, "deliverynote"),
            Self::Orderconfirmation => write!(f, "orderconfirmation"),
            Self::Quotation => write!(f, "quotation"),
            Self::Proformainvoice => write!(f, "proformainvoice"),
        }
    }
}

impl Default for DocumentType {
    fn default() -> DocumentType {
        Self::Invoice
    }
}

