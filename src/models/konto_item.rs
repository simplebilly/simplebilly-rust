/*
 * SimpleBilly API
 *
 * Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.
 *
 * The version of the OpenAPI document: 0.1.0
 * Contact: support@simplebilly.com
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct KontoItem {
    #[serde(rename = "anfangsbestand")]
    pub anfangsbestand: String,
    #[serde(rename = "haben_umsatz")]
    pub haben_umsatz: String,
    #[serde(rename = "konto")]
    pub konto: String,
    #[serde(rename = "name")]
    pub name: String,
    #[serde(rename = "saldo")]
    pub saldo: String,
    #[serde(rename = "soll_umsatz")]
    pub soll_umsatz: String,
}

impl KontoItem {
    pub fn new(anfangsbestand: String, haben_umsatz: String, konto: String, name: String, saldo: String, soll_umsatz: String) -> KontoItem {
        KontoItem {
            anfangsbestand,
            haben_umsatz,
            konto,
            name,
            saldo,
            soll_umsatz,
        }
    }
}

